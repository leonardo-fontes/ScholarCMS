...
export default function ProfilePage() {
	...
	const isExternalProfile = user && id !== undefined ? parseInt(id) !== parseInt(user.id) : true;
	...
	useEffect(() => {
		if (isExternalProfile && id)
			getProfileExternal(id);
		console.log("publicationId" + publicationId)
		publicationId ? fetchPublication(publicationId) : console.log("Aqui é falso");
	...
	return (
		<div className={`flex flex-col w-full items-center ${isExternalProfile ? 'backdrop-blur-sm' : ''}`}>
			<div className="flex">
				<section className="w-full">
					<section className="flex max-w-[560px] w-full mb-4 gap-12 mt-20 items-center justify-between relative">
						{isExternalProfile ? (
							...
							<div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black bg-opacity-50">
								<Payment username={userExternal && userExternal?.name} className="relative z-50 p-8 bg-white rounded-lg shadow-lg" />
							</div>
						) : (
						...
						{!isExternalProfile && //adicionar ! (negação) na validação
							<div className="flex flex-col items-center justify-center gap-4 mt-4">
								<Button classname="bg-primaryLight"
									children={<div className="flex gap-2 items-center justify-center w-52 p-2 text-primary rounded-md"
					...
					{!isExternalProfile &&
						<p className="bg-white rounded-md p-4 max-w-[560px] border-[1px] border-primary ">
							Sou o Vinnicius, um entusiasta da tecnologia e inovação, atualmente cursando Análise e Desenvolvimento de Sistemas. Morando em Cubatão, me dedico a aprender e aplicar meus conhecimentos para criar soluções que possam beneficiar a comunidade.
							Estou sempre em busca de novos desafios e oportunidades para contribuir com causas importantes.						</p>
					}
				</section>
				<div className="flex justify-center gap-10 fixed right-7">
					{isProfilePicFormOpen && <ProfilePicForm onClose={closeProfilePicForm} onUpdatePicture={handleUpdatePicture} />}
				</div>
			</div>
			{publication ?
			...
			{!publication && user?.role_id === parseInt(Role.Beneficiario) && publication == 0 &&
				<div className="max-w-[560px] mt-20 pt-20 border-t-[1px] border-primaryLight">
					<div className="flex flex-col items-center justify-center bg-primaryLight text-primary text-xl rounded-md p-4 text-center">
						...
						<Button classname="my-12 text-lg md:text-2xl font-semibold md:font-bold text-primary bg-white hover:text-white hover:bg-primary w-[380px] md:py-2" href="/platform/create-pub" children={'CRIAR PUBLICAÇÃO'} />
					</div>
				</div>}

			<Button classname="my-12 text-lg md:text-2xl font-semibold md:font-bold text-white bg-primary hover:text-primary hover:bg-white w-[500px] md:py-2" href="/" children={'VOLTAR PARA A PÁGINA INICIAL'} />
			{isExternalProfile && (
				<Payment username={userExternal ? userExternal.name : "undefined"} />)}
