...
export default function Publication({ post, comments }: PublicationType) {
    ...
    const [userInternalPicture, setUserInternalPicture] = useState<string | undefined>(undefined);
    const { user } = useAuth();
    const [photoPublication, setPhotoPublication] = useState<string | undefined>(undefined);
    ...
    useEffect(() => {
        const fetchPhotos = async () => {
            ...
            if (post?.user_picture) {
                ...
                if (userPictureUrl)
                    setUserInternalPicture(userPictureUrl);
            }
            setIsLoading(false);
        }
        fetchPhotos();
    ...
    useEffect(() => {
        const fetchCommentPictures = async () => {
            if (comments && comments.length > 0) {
                const validComments = comments.filter(comment => comment.user_picture)
